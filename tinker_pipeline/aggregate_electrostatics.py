...
def main() -> None:
    ...
    perfold_path = out_dir / "sweep_electrostatics_138_perfold.csv"
    perfold.to_csv(perfold_path, index=False)
    print(f"Wrote {perfold_path}  ({len(perfold)} rows)")

    # Per-variant summary: mean/std MAE across (seed, fold)
    ...
    # Paired Wilcoxon vs Charge baseline, on (seed, fold) keys
    ...
    for v in VARIANTS:
        ...
        if len(merged) < 2:
            deltas.append(dict(variant=v, n_pairs=len(merged), mean_delta_MAE=np.nan,
                               wilcoxon_p=np.nan, ttest_p=np.nan, frac_better=np.nan))
            continue
        delta = merged["MAE"].values - merged["MAE_base"].values  # negative = better
        try:
            w_stat, w_p = wilcoxon(delta, alternative="two-sided", zero_method="wilcox")
        except ValueError:
            w_p = np.nan
        try:
    ...
    summary_path = out_dir / "sweep_electrostatics_138_summary.csv"
