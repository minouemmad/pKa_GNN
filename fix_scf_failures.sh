...
# ── Step 4a: Final — no polarization ──────────────────────────────────────────
echo "=== Step 4a: Final Minimize polarization=NONE (RMS=1.0) ==="
${FFX} Minimize -e 1.0 "${pdb_s4_in}" ${PROPS} -Dpolarization=NONE
if [ ! -f "${pdb_s4_in}_2" ]; then
    echo "ERROR: Step 4a output not found: ${pdb_s4_in}_2"; exit 1
fi
mv "${pdb_s4_in}_2" "${BASE}/PDBNAME_s4a.pdb"

# ── Step 4b: Final — direct polarization ──────────────────────────────────────
echo "=== Step 4b: Final Minimize polarization=DIRECT (RMS=0.5) ==="
${FFX} Minimize -e 0.5 "${BASE}/PDBNAME_s4a.pdb" ${PROPS} -Dpolarization=DIRECT
if [ ! -f "${BASE}/PDBNAME_s4a.pdb_2" ]; then
    echo "ERROR: Step 4b output not found: ${BASE}/PDBNAME_s4a.pdb_2"; exit 1
fi
mv "${BASE}/PDBNAME_s4a.pdb_2" "${BASE}/PDBNAME_s4b.pdb"

# ── Step 4c: Final — mutual polarization, tight convergence, save dipoles ─────
echo "=== Step 4c: Final Minimize polarization=MUTUAL (RMS=0.1, saveInduced) ==="
${FFX} Minimize -e 0.1 "${BASE}/PDBNAME_s4b.pdb" ${PROPS} -Dpolarization=MUTUAL --saveInduced
if [ ! -f "${BASE}/PDBNAME_s4b.pdb_2" ]; then
    echo "ERROR: Step 4c output not found: ${BASE}/PDBNAME_s4b.pdb_2"; exit 1
fi
mv "${BASE}/PDBNAME_s4b.pdb_2" "${BASE}/PDBNAME_final.pdb"
